...
Glyphs that fall outside of this specified glyph range, are autohinted, but are not assigned cvt’s. A YAnchor command is typically applied to the y-min and y-max of these glyphs, but without a reference to a cvt. 
...
**Problem:** Global adjustments _(to key Heights cvt's for example,)_ are often made in the cvt table to adjust the overall hinted proportions of the fonts. These adjustments only apply to glyphs that have been assigned height cvt’s. Glyphs that fall outside of the VTT supported glyph set, require manual hinting and appropriate cvt’s assigned to ensure consistency with the main glyph set.
