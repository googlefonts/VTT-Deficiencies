...
Extending Character group support for addition glyph sets, and scripts besides Latin.

The CharGrp.txt file for the font assigns each glyph to a character group: uppercase, lowercase, figure, or other. This file is imported into VTT when preparing a font for hinting. This data is also used internally by the VTT Autohinter, when Autohinting a font.

The ‘character group’ tells Visual TrueType which group of values to use from the control value table (‘cvt’) for the glyph. The VTT Autohinter uses character group data to assign cvt’s, to glyphs, identifying common heights for Latin, Greek and Cyrillic, (WGL4 set that is currently supported internally in the Autohinter)

Glyphs that fall outside of this specified glyph range, are autohinted, but are not assigned cvt’s. A y-anchor command is typically applied to the y-min and y-max of these glyphs, but without a reference to a cvt. 
...
**Note:** The addition of further character groups supports manual hinting, and speeds up the hinting process, by allowing VTT to automatically choose the correct cvt’s as the glyph is hinted using the graphcigraphical hinting tools. There is no way currently to have the Autohinter recognise these new character groups, and apply the correct cvt’s automatically. 

**Problem:** Global adjustments _(to key Heights cvt's for example,)_ are often made in the cvt table to adjust the overall hinted proportions of the fonts. These adjustments will only apply to glyphs that have been assigned height cvt’s. To ensure that glyphs that fall outside of the VTT supported glyph set, maintain consistency, they need manual hinting and appropriate cvt’s assigned. 

**Example:** Ligature glyphs, _(ffi, ffl, ft, fft,)_ are not recognised as lowercase, by the Autohinter and are assigned into the default ‘other’ character group, without any cvt’s being assigned for heights. Global adjustments made to change the x-height or ascender height for the main font, will not be reflected in the ligature glyphs until they have been assigned cvt’s to match the lowercase. Many newer fonts contain much larger glyphs sets, including extensive Opentype sets (ligatures, Small Caps, with extended accented sets). Currently all of these glyphs require manual adjustments, and appropriate cvt assignment, a time consuming process.

**Recommendation**
Investigate extending the Autohinter to recognise additional glyph sets outside of the currently supported WGL4 set

Investigate extending the Autohinter to recognise addition glyph ranges for other scripts. This would require additional script specific typographic height information to be built into the autohinter. There is a possibility to leverage come relevant script specific work already [completed for building the TTFAutohinter](https://freetype.org/ttfautohint/doc/ttfautohint.html#scripts)